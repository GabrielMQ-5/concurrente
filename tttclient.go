...
func isWinner(t byte, w byte) bool {
	var l byte
	if w == 111 {
		l = 120
	} else if w == 120 {
	...
	if t == w {
		fmt.Println("Felicidades, usted ha ganado!")
		return true
	} else if t == l {
		fmt.Println("Mejor suerte la proxima vez!")
		return true
	}
	return false
}
...
func main() {
	...
	for !gameover {
		...
		r.Read(buff)
		dibujaTablero(getTab(buff))
		scanJugada(getTab(buff), piece)
		winner := findWinner(getTab(buff))
		dibujaTablero(getTab(buff))
